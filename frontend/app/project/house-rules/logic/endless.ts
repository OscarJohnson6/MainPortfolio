// logic/endless.ts — Acts configuration, House Heat, endless mode scaling
...
/** Closing Time pressure — base starting value based on act. */
export function getBaseClosingTime(actIdx: number): number {
  if (actIdx < 3) return 0;
  return Math.floor((actIdx - 2) * 8);
}
